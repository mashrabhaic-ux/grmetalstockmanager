...
# 1. APPLICATION SETUP
st.set_page_config(page_title="GR Metal Corporation", page_icon="⚙️", layout="wide")
st.title("🏢 GR METAL CORPORATION — Warehouse Management System")

# SYSTEM POLISH: Visual Sub-header Badge
st.markdown(
    """
    <div style="background-color:#1e293b; padding:12px; border-radius:6px; margin-bottom:20px;">
        <h4 style="color:#ffffff; margin:0; font-size:14px; font-weight:normal;">🏭 Active Session Mode: Connected to Live Factory Floor Register</h4>
    </div>
    """, 
...
# 4. INITIALIZE DATABASES (With column verification checks)
...
# SAFETY RESET: If 'date_logged' is missing due to old memory cache, force reset data frame cleanly
...
st.sidebar.markdown("---")
st.sidebar.subheader("⚠️ Warehouse System Actions")
if st.sidebar.button("🗑️ Reset Application Database", help="Clear out data records back to empty state templates"):
...
# Sync active state handles safely
...
tab1, tab2 = st.tabs(["📋 FULL SHEETS INVENTORY", "✂️ OFF-CUT BITS DATABASE"])
    ...
    with fs_col2:
        ...
        if not df_sheets.empty:
            ...
            if sheet_options:
                ...
                # Slicing Allocation Map Preview
                display_scale = 0.15
                ...
                if st.button("🔥 Execute Cut Order", use_container_width=True):
                    ...
                    new_bits_to_add = []
                    if rem_len > 0:
                        new_bits_to_add.append({"length": rem_len, "width": parent_row['width']})
                    if rem_wid > 0 and cut_len > 0:
                        new_bits_to_add.append({"length": cut_len, "width": rem_wid})
                    ...
                    st.success("Parent sheet decremented and remnants added cleanly to the database!")
                    st.rerun()
            else:
                st.info("No parent full sheets available in stock to cut.")
...
with tab2:
    ...
    with bc1:
        ...
        with st.form("manual_bit_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Save Bit to Stock"):
                new_bit = {
                    "bit_id": m_id, "material": m_mat, "grade": m_grade, "thickness": m_thick, 
                    "length": m_len, "width": m_wid, "weight_kg": b_weight, "status": "Available",
                    "date_logged": datetime.date.today().strftime("%d-%b-%Y")
                }
    ...
    with bc2:
        ...
        if not df_bits.empty:
            matches = df_bits[
                (df_bits['material'] == req_mat) & 
                (df_bits['thickness'].round(2) == round(req_thick, 2)) & 
                (df_bits['length'] >= (req_len - tolerance_mm)) & 
                (df_bits['width'] >= (req_wid - tolerance_mm)) & 
                (df_bits['status'] == "Available")
            ]
            if not matches.empty:
                sorted_matches = matches.sort_values(by=['length', 'width'])
                st.success(f"🎉 Found {len(sorted_matches)} matching piece(s) using a {tolerance_mm}mm buffer margin limit!")
                ...
                whatsapp_msg = f"⚙️ *GR METAL CORPORATION* ⚙️\n\n" \
                               f"• *Material:* {best_match['material']}\n" \
                               f"• *ID:* {best_match['bit_id']} ({best_match['grade']})\n" \
    ...
    st.markdown("---")
    st.subheader("🛠️ Quick Off-Cut Actions & Visual Layout Controls")
    if df_bits.empty:
